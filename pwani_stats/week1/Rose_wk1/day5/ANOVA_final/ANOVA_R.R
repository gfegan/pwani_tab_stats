## this Rfile introduces the students to the basics of ANOVA in R and is restricted upto a basic two-way ANOVA
setwd("/Users/lwafula/KWTRP/person_profiles&requests/Pwani_university/ANOVA")
...
#critical values for F-distribution with 3, and , 5 df at 95% CI  at the respective quantile
qf(.95,df1=3,df2=13)
qf(.99, df1=3,df2=12)

#critical values at 99% CI at the respective quantile
qf(.99, df1=4, df2=6)
qf(.99, df1=6, df2=22)
...
##### drug, the means of systolic across different drugs
...
oneway.test(systolic~drug, var.equal = TRUE)
...
#diagnostics
qqnorm(lm1$res, pch=16) ##check on normality of our residuals
...
##TWO-WAY anova
##used to compare the means of populations that are classified in two diff. ways: could be randomised block design and/or factorial design
use (data)
data
...
##for each treatment on the y-axis. In addition, a line connecting the means corresponding to each level of second factor. 
##When no interaction is present, the lines are roughly parallel
...
print(model.tables(lm_3_2,"means"),digits=3)       #report the means and the number of subjects/cell
...
##an alternative way to fit the two-way ANOVA is
...
summary(lm3)
anova(lm3)   ##check whether the p-value for interaction term is significant, look for the other vars, look for the main effects significance
...
qqnorm(lm3$res)  ##check how they compare with normal quantiles of the residuals
...
#####################################################
###Practical2 for self_tests for the students
data1<-data.frame(
yields=
c(5, 7, 3,
4, 2, 6,
